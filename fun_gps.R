...
# It assumes columns named subid, date, lat, lon, cnt_pts, time_next

  places <- places %>%
    mutate(time_next = as.numeric(time_next))
  ...
  if (nrow(places) > 1) {
    ...
    avg_place <- tibble(date = Date(), lat = double(), lon = double(), cnt_pts = double(), time_next = double())
    for (i in 1:max_grp) {
      ...
      if (nrow(places_grouped) > 1) {
        ...
        avg_place <- avg_place %>%
          add_row(date = max(places_grouped$date),  # most recent date
                  ...
                  time_next = sum(places_grouped$time_next))

      } else {
        avg_place <- avg_place %>%
          add_row(select(places_grouped, date, lat, lon, cnt_pts, time_next))
      }
    }
  ...
  } else  avg_place <- places %>% select(subid, date, lat, lon, cnt_pts, time_next)
