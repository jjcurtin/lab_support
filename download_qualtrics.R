...
# aspects of his code that were unnecessary for our usage.
  ...
  result <- qualtrics_api_request(api_key, "POST", URL = root_url, body = raw_payload)
  ...
  survey_path <- download_qualtrics_export(check_url, api_key)
...
qualtrics_api_request <- function(api_key, verb = c("GET", "POST"), URL,
                               body = NULL) {
...
# It is not intended to be called directly.
  ...
  # Construct header
  headers = constructHeader(api_key)
...
constructHeader <- function(api_key) {
...
download_qualtrics_export <- function(check_url, api_key) {
...
# It is not intended to be called directly.
  ...
  while(progress < 100) {
    ...
    CU = qualtrics_api_request(api_key,"GET", URL = check_url)
