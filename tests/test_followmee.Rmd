...
switch (Sys.info()[['sysname']],
        ...
        Windows = {
          ...
          path_out <- "P:/studydata/snezana/raw_data"},                       #are we going to be saving these CSV this way?

        # IOS paths
        Darwin = {
          path_admin <- "/Volumes/private/administration"
          path_lab_support <- "/Volumes/toolboxes/lab_support"
          path_out <- "/Volumes/private/studydata/snezana/raw_data"}
        )
```
...
gps <- updated_points %>% 
  select(subid, time = date, lat, lon) %>%                              #JOHN do we need to use any speed/accuracy info or is it enough as is?
  # mutate(time = round(as.numeric(time)),                              #ALREADY FORMATTED CORRECTLY UNNEEDED
...
end_date <- start_date + months(1)
                                                                         ...
                                                                         #this relied on fun_star and survey results; will need to modify for Snezana
places_past<- tibble()
