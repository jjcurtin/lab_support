...
### Refit best model for each algorithm and feature set
...
```{r warning = FALSE}
# warning set to false to hide warning that there are no tuning parameters provided
# this is okay and occurs because we are only providing the best hyperparameter value
# but still using fits to fit across folds (replicating CHTC process)
for (k in algorithms) {
  ...
  for (i in feature_sets) {
    ...
    best_model <- slice_max(results_i, get(perf_metric)) %>% 
      glimpse()
    ...
    # save out model
    write_rds(best_model_fits, here(path_job, str_c("output/best_", k, "_", i, "_", "model_fits.rds")), compress = "xz")
    
    # save out predictions
    write_rds(predictions, here(path_job, str_c("output/best_", k, "_", i, "_", "model_preds.rds")), compress = "xz") 
    
    # save out results
    write_csv(best_model_results, here(path_job, str_c("output/best_", k, "_", i, "_", "model_results.csv")))
  }
}
```


### best overall model predictions
...
Read in predictions
```{r}
k <- best_model$algorithm
i <- best_model$feature_set

predictions <- read_rds(here(path_job, "output/best_", k,  "_", i, "_", "model_preds.rds"))
```
...
### View top predictors by algorithm and feature set

Functions for vi permutation
```{r}
...
pred_perm <- function(object, newdata) {
  preds <- predict(object, newdata, type = "response", s = object$fit$penalty)[, 1]    
...
for (k in algorithms) {
  ...
  for (i in feature_sets) {
    ...
    # For some reason id variables in recipe were being included as predictors so
    # updating recipe to remove them 
    rec <- rec %>% 
    ...
    if (best_model$algorithm == "glmnet") {
      ...
      # view non-zero coefficients as a check
      model %>% 
        tidy() %>% 
        filter(estimate != 0) %>% 
    ...
    # separate x and y for permutation
    x <- feat_all %>% 
      select(-y) %>% 
      as.matrix()
    y <- feat_all %>% 
